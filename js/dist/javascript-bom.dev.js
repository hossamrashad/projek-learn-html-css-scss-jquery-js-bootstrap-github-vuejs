...
video11div.textContent = window.innerHeight + " = innerHeight   بيجيب مساحة الوندو ملهوش دعوة بالاكنسال بيجيب المساحة الفعلية للشاشة ";
video12div.textContent = window.outerHeight + " = outerHeight  بيجيب مساحة الوندو من غير الكنسال لاج ";
video13div.textContent = window.innerWidth + " = innerWidth   بيجيب مساحة الوندو ملهوش دعوة بالاكنسال بيجيب المساحة الفعلية للشاشة ";
video14div.textContent = window.outerWidth + " = outerWidth  بيجيب مساحة الوندو من غير الكنسال لاج "; // video 3
...
var video31div = document.getElementById("video31div"),
    ...
    video35div = document.getElementById("video35div"); // طريقة انو انت تعمال سطر جديد فى الارارت
//  alert("hossam \nrashad");
// confirm
...
}, 3000); // طريقة تانية لكتابة الست تيم اوت
...
setTimeout(video42test, 4000); // clear time out
...
setInterval(video52test, 4000); // clear setInterval
...
var myTime = setInterval(countDown, 1000); // video 6 || window open || الاعلانات
...
video6btn.onclick = function () {
  "use strict"; // لو مكتبتش المسار اللى هيروح فية بيفتح صفحة فاضية جديدة
...
video61btn.onclick = function () {
  ...
  window.open("https://www.google.com", "_blank", "width=1000,height=1080,menubar=no,status=no", "true");
...
var video81div = document.getElementById("video81div"),
    ...
    video81btn = document.getElementById("video81btn"); // window.stop();
// window.close();
// window.focus();
...
video105div.textContent = scrollY; // video 11  location => href
...
video111btn.onclick = function () {
  "use strict"; // كدا انا غيارت مسار الصفحة اللى انا فيها الى صفحة تانية
...
video131div.textContent = window.location.protocol + " protocol"; // video 14 replace || assign || reload
...
video15div.textContent = window.history.length; // video 16  navigator
...
video162div.textContent = window.navigator.appName + "  || appName"; // الاصدار
...
video168div.textContent = window.navigator.product + " || product"; // video 17 window screen
...
video175div.textContent = window.screen.colorDepth + "  درجة الالوان   "; // video175div.textContent = window.screen.colorDepth + "  درجة الالوان   ";

video175div.textContent = window.screen.pixelDepth + "  درجة الالوان   "; // video 18 cookies
// cookies هو حفظ الاعدادات اللى فى الصفحة على جهازاك 
...
video181div.textContent = document.cookie; // video 19
...
var video191div = document.getElementById("video191div"),
    ...
    video191btn = document.getElementById("video191btn"); // انشاء الكوكاز
